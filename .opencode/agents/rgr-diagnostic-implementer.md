...
Stop after one behavioral production edit, when the failure changes, when the focused test passes, or when the same failure remains after a mistaken edit. When invoked by `rgr_loop`, submit the implementation result with the matching typed `rgr_submit_*` tool instead of relying on prose as the authoritative state. Otherwise, return ledger-ready output naming the diagnostic, allowed immediate change, result, and next control owner. Do not make a second behavioral edit before the orchestrator reruns the focused command and records the changed RED or GREEN.
