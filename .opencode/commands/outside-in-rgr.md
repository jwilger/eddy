...
Run the outside-in RGR workflow for: $ARGUMENTS

Prefer the `opencode-rgr-loop` plugin's `rgr_loop` tool. Provide it a BDD-style behavioral specification and let its typed submissions own the RGR ledger.

If `rgr_loop` is unavailable or cannot fit the task, use the `outside-in-rgr-microcycle` skill and keep a visible fallback ledger. The primary implementer orchestrates; the RED/GREEN/review agents own their steps. Do not skip from RED to broad implementation, and do not accept multi-failure RED output.

Workflow:

1. Dispatch `rgr-test-author` to write or activate the next smallest failing test and capture exactly one RED failure.
2. Dispatch `rgr-test-reviewer` and record RED approval before any production edit.
...
6. After one behavioral production edit, rerun the focused command. When the failure changes, record the new RED in the handoff and return control to the orchestrator.
